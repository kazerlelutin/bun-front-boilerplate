...
describe('translate.store', () => {
  ...
  beforeEach(() => {
    ...
    mockElement.setAttribute('data-translate', 'home');
  ...
  describe('setCurrentLanguage', () => {
    ...
    it('should update DOM elements with data-translate attribute', () => {
      ...
      expect(mockElement.textContent).toBe(UI.home.en);
    });

    it('should update DOM elements when switching to French', () => {
      setCurrentLanguage('fr');

      expect(mockElement.textContent).toBe(UI.home.fr);
    });

    it('should update DOM elements when switching to Korean', () => {
      setCurrentLanguage('ko');

      expect(mockElement.textContent).toBe(UI.home.ko);
    });

    it('should handle multiple elements with data-translate', () => {
      const element2 = document.createElement('span');
      element2.setAttribute('data-translate', 'about');
      ...
      expect(mockElement.textContent).toBe(UI.home.en);
      expect(element2.textContent).toBe(UI.about.en);
    });
